...
- Number of edges skipped (target not in user list)
...
- `network.gml` — undirected weighted graph in GML format
  - Node attributes: `playcount` (int), `top_artists` (pipe-delimited string), `top_tags` (pipe-delimited string)
...
- If a large number of edges are skipped (e.g. >50%), warn the user that the scraped user set may be too small relative to the interaction data.
